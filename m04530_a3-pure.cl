...
KERNEL_FQ void m04530_mxx (KERN_ATTR_VECTOR ())
{
  /**
   * modifier
   */
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos += VECT_SIZE)
  {
    ...
    const u32x a2 = ctx.h[0];
    const u32x b2 = ctx.h[1];
    const u32x c2 = ctx.h[2];
    const u32x d2 = ctx.h[3];
    const u32x e2 = ctx.h[4];

    sha1_ctx_vector_t ctxf;

    sha1_init_vector_from_scalar (&ctxf, &ctx0);
    ...
    w0f[0] = uint_to_hex_lower8_le ((a2 >> 16) & 255) <<  0
           | uint_to_hex_lower8_le ((a2 >> 24) & 255) << 16;
    w0f[1] = uint_to_hex_lower8_le ((a2 >>  0) & 255) <<  0
           | uint_to_hex_lower8_le ((a2 >>  8) & 255) << 16;
    w0f[2] = uint_to_hex_lower8_le ((b2 >> 16) & 255) <<  0
           | uint_to_hex_lower8_le ((b2 >> 24) & 255) << 16;
    w0f[3] = uint_to_hex_lower8_le ((b2 >>  0) & 255) <<  0
           | uint_to_hex_lower8_le ((b2 >>  8) & 255) << 16;
    w1f[0] = uint_to_hex_lower8_le ((c2 >> 16) & 255) <<  0
           | uint_to_hex_lower8_le ((c2 >> 24) & 255) << 16;
    w1f[1] = uint_to_hex_lower8_le ((c2 >>  0) & 255) <<  0
           | uint_to_hex_lower8_le ((c2 >>  8) & 255) << 16;
    w1f[2] = uint_to_hex_lower8_le ((d2 >> 16) & 255) <<  0
           | uint_to_hex_lower8_le ((d2 >> 24) & 255) << 16;
    w1f[3] = uint_to_hex_lower8_le ((d2 >>  0) & 255) <<  0
           | uint_to_hex_lower8_le ((d2 >>  8) & 255) << 16;
    w2f[0] = uint_to_hex_lower8_le ((e2 >> 16) & 255) <<  0
           | uint_to_hex_lower8_le ((e2 >> 24) & 255) << 16;
    w2f[1] = uint_to_hex_lower8_le ((e2 >>  0) & 255) <<  0
           | uint_to_hex_lower8_le ((e2 >>  8) & 255) << 16;
    w2f[2] = 0;
    w2f[3] = 0;
    w3f[0] = 0;
    w3f[1] = 0;
    ...
    sha1_update_vector_64 (&ctxf, w0f, w1f, w2f, w3f, 40);

    sha1_final_vector (&ctxf);

    /* ================== */

    const u32x r0 = ctxf.h[DGST_R0];
    const u32x r1 = ctxf.h[DGST_R1];
    const u32x r2 = ctxf.h[DGST_R2];
    const u32x r3 = ctxf.h[DGST_R3];

    COMPARE_M_SIMD (r0, r1, r2, r3);
  }
}

KERNEL_FQ void m04530_sxx (KERN_ATTR_VECTOR ())
{
  /**
   * modifier
   */
