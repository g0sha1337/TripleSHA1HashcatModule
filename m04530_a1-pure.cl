...
DECLSPEC void m04530_hash_core (PRIVATE_AS const u32 *pw_buf,
                                const u32 pw_len,
...
KERNEL_FQ void m04530_mxx (KERN_ATTR_BASIC ())
{
  ...
  const u32 pw_len_l = pws[gid].pw_len;
  u32 w_l[16];
  for (int i = 0; i < 16; i++) w_l[i] = pws[gid].i[i];

  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos++)
  {
    const u32 pw_len_r = combs_buf[il_pos].pw_len;
    u32 w_r[8];
    for (int i = 0; i < 8; i++) w_r[i] = combs_buf[il_pos].i[i];

    u32 pw_buf[16] = { 0 };
    for (int i = 0; i < 16; i++) pw_buf[i] = w_l[i];

    const u32 offset = pw_len_l / 4;
    const u32 shift  = (pw_len_l % 4) * 8;

    if (shift == 0)
    {
      for (int i = 0; i < 8 && (offset + i) < 16; i++)
        pw_buf[offset + i] |= w_r[i];
    }
    else
    {
      const u32 shift_rev = 32 - shift;
      u32 carry = 0;
      for (int i = 0; i < 8 && (offset + i) < 16; i++)
      {
        u32 tmp = w_r[i];
        pw_buf[offset + i] |= (tmp << shift) | carry;
        carry = tmp >> shift_rev;
      }
      if (offset + 8 < 16) pw_buf[offset + 8] |= carry;
    }

    const u32 combined_len = pw_len_l + pw_len_r;

    u32 r0, r1, r2, r3, r4;
    m04530_hash_core (pw_buf, combined_len, salt_bufs, kernel_param, &r0, &r1, &r2, &r3, &r4);
